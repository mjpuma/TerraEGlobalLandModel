...
!#define AQUIFER
!#define AQUIFER_NO_HEAT_FLUX
!#define AQUIFER_NO_WATER_FLUX
!-----------------------------------------------------------------------
...
#ifdef AQUIFER
!@var Sy_aq aquifer specific yield
!@var dz_aq aquifer thickness (m) below soil layers
      real*8, parameter, public :: Sy_aq = 0.2, dz_aq = 50.d0
#endif
...
#ifdef AQUIFER
!@var w_aq aquifer water volume per unit surface area (m)
!@var ht_aq aquifer water heat content per unit surface area (J/m^2)
      real*8, public :: w_aq, ht_aq
!@var ptype  fraction of cell area that has soil (not ice or lake)
      real*8 :: ptype
#endif
...
ccc   soil internal variables which need to be passed from/to ghinij
      real*8 :: zb(ng),zc(ng),fr(ng),snowm !veg alaie, rs,
     &     ,thets(0:ng,2),thetm(0:ng,2),ws(0:ngm,2),shc(0:ng,2)
     &     ,thm(0:64,imt-1)
#ifdef AQUIFER
!@var specific heat capacity of aquifer regolith/rock (J/m^2 / K)
      real*8 :: shc_aq
!@var zb_aq aquifer bottom elevation relative to surface (m)
      real*8 :: zb_aq
#endif
      public thm, thets, ws
...
#ifdef AQUIFER
!@var tp_aq aquifer temperature, degC
!@var fice_aq aquifer fraction frozen
      real*8, public :: tp_aq, fice_aq
#endif
...
ccc   coefficients (or data needed to compute them)
     ...
     &     ,d(0:ng,2) ,xku(0:ng,2)
      ...
      public h,xku !not for global

!@var diagnostic soil flux accumulators
      real*8,public,dimension(ng,2) ::accum_water_flux, accum_heat_flux

#ifdef AQUIFER
!@var h_aq aquifer water table (=head), m
      real*8, public :: h_aq
#endif

ccc   the following are fluxes computed inside the snow model
ccc   they are unit/(m^2 of snow), not multiplied by any fractions
...
!@var evapdl transpiration from each soil layer
      real*8 :: evapdl(ngm,2)
!@var albedo
      real*8 :: albedo_cell
...
C***
     ...
     &     ,accum_water_flux,accum_heat_flux
#ifdef AQUIFER
     &     ,fice_aq,h_aq,ht_aq,shc_aq,tp_aq,w_aq,zb_aq
#endif
     &     ,ijdebug,n,nsn !nth
...
!-----------------------------------------------------------------------
      ...
      subroutine hydra
!@sum computes matr. potential h(k,ibv) and H2O conductivity xk(k,ibv)
...
!-----------------------------------------------------------------------
      ...
      subroutine evap_limits(time_step,
#ifndef USE_ENT
     &     vegcell,
#endif
...
ccc !!! it''s a hack should call it somewhere else !!!
     ...
     &         (w(k,ibv)-dz(k)*thetm(k,ibv))/time_step
        enddo
      enddo
     ...
     &       2.467d0*d(1,1)*(theta(1,1)-thetm(1,1))/dz(1) + pr )
      ...
      if ( process_vege ) then
        ibv = 2
        evap_max_wet(ibv) = w(0,2)/time_step !+ pr ! pr doesn''t work for snow
...
c     bare soil evaporation
      if ( process_bare ) then
c      Keep surface evaporation to the available water in the top soil layer 
c      Nir, 6/2011        
        evap_max_dry(i_bare) = min(evap_max_dry(i_bare),
     &    (w(1,i_bare)-dz(1)*thetm(1,i_bare))/time_step)
     
        evapb = min( epb, evap_max_dry(i_bare) )
        evapb = max( evapb, -qm1dt )
        evapbs = min( epbs, evap_max_snow(i_bare) )
        evapbs = max( evapbs, -qm1dt )
!      evapor(i_bare) = fr_snow(i_bare)*evapbs + (1.-fr_snow(i_bare))*evapb
      else
...
c     evapvw is wet evaporation from canopy (from interception)
        ...
        evapvw = min( epv, evap_max_wet(i_vege) )
        evapvw = max( evapvw,-qm1dt )
        evapvd = min(epv1,evap_max_dry(i_vege)) !evap_max_dry(i_vege) depends on qs
...
c     compute transpiration for separate layers (=0 for bare soil)
        ...
        if ( betad > 0.d0 ) then
          do k=1,n
            evapdl(k,i_vege) = evapvd*betadl(k)/betad
          end do
        endif

c     Reduce water taken from soil layers so that soil water does not 
c     drop below its minimum value (Nir, 6/2011)
        do ibv=i_bare,i_vege
          do k=1,n
            evapdl(k,ibv)=min(evapdl(k,ibv), 
     &                        (w(k,ibv)-dz(k)*thetm(k,ibv))/time_step)
          enddo
        enddo
c     Adjust the total evaporation accordingly  
        evapvd = 0.d0
        do k=1,n
          evapvd = evapvd + evapdl(k,i_vege)
        enddo     
        evapvs = min( epvs, evap_max_snow(i_vege) )
        evapvs = max( evapvs, -qm1dt )

#ifdef EVAP_VEG_GROUND
c      Keep surface evaporation to the available water in the top soil layer 
c      Nir, 6/2011
        evap_max_vegsoil = min(evap_max_vegsoil,
     &   (w(1,i_vege)-dz(1)*thetm(1,i_vege))/time_step-evapdl(1,i_vege))

        evapvg = min(epvg,evap_max_vegsoil)
        evapvg = min(evapvg,evap_max(i_vege) - evapvd*fd)
        ! keep evapv <= epv
...
!     &     ( theta(0,2)*evapvw + (1.-theta(0,2))*evapvd )
      ...
      ! Bare Soil so set veg related ET fluxes to zero
...
c**** revised; Nir Krakauer / M. Puma, June 2011
c**** input:
c**** w - water in layers, m
c#ifdef AQUIFER
c**** w_aq - aquifer water, m
c**** fice_aq - aquifer fraction frozen
c**** ptype - fraction of cell area that has soil (not ice or lake)
c#endif
c**** thetm - minimum volumetric moisture in layers
c**** ws - saturated water in layers, m
c**** dts - current time step size, s
c**** f - water fluxes between layers, m s-1
c**** evapdl - evaporation from layers, m s-1
c**** rnf - surface runoff, m s-1
c**** rnff - subsurface runoff, m s-1
c**** fr_snow - fraction snow cover
c**** fm - snow masking fraction for canopy (=1 if all snow)
c**** fd - effective fraction of dry canopy (=0 if dew)
c**** output:
c**** f - limited water fluxes, m s-1
c**** rnf - limited surface runoff, m s-1
c**** rnff - limited subsurface runoff, m s-1
c**** evapdl - limited evaporation from layers, m s-1
      real*8 :: dflux,drnf
      integer k, ibv
      real*8 :: wn

#ifdef AQUIFER
ccccccc AQUIFER LIMITING STEP 1   
ccc   No flow from aquifer if it is totally frozen
ccc   (this is needed to ensure heat conservation)
      if (fice_aq == 1.d0) then
        f(n+1,:) = min(f(n+1,:), 0.d0)
      endif

ccccccc AQUIFER LIMITING STEP 2
ccc   Prevent unrealistic transfers of water between aquifer and soil due 
ccc   to too-large time step; specifically, keep the amount transferred 
ccc   out in one timestep to half the water content in the aquifer

!    Total aquifer-to-soil flux
      dflux = ptype*dts*(fb*f(n+1,i_bare) + fv*f(n+1,i_vege))
      if (dflux > 5d-1*w_aq) then
!      Apply multiplier to decrease flux
        f(n+1,:) = ((5d-1*w_aq) / dflux)*f(n+1,:)
      endif  

ccccccc AQUIFER LIMITING STEP 3      
ccc   Prevent the aquifer from overflowing into the soil layers;
ccc   also prevent the aquifer from becoming completely depleted
ccc   (unlikely eventuality, but worth guarding against)

!    Total aquifer-to-soil flux
      dflux = ptype*dts*(fb*f(n+1,i_bare) + fv*f(n+1,i_vege))
      if (dflux == 0.d0) then
         f(n+1,:) = 0.d0
      elseif(dflux < w_aq-(Sy_aq*dz_aq)) then !Decrease soil-to-aquifer flux
!      Apply multiplier to decrease flux
        f(n+1,:) = ((w_aq-(Sy_aq*dz_aq))/dflux)*f(n+1,:) 
      elseif (dflux > w_aq-1d-6) then      !Decrease aquifer-to-soil flux
!      Apply multiplier to decrease flux
        f(n+1,:) = ((w_aq-1d-6)/dflux)*f(n+1,:)
      endif

ccccccc AQUIFER LIMITING STEP 4
ccc   Eliminate soil to aquifer flux if soil layer would become too dry 
      do ibv=i_bare,i_vege
         wn = w(n,ibv) +( f(n+1,ibv) - max(f(n,ibv), 0.d0)
     &       - rnff(n,ibv) - fd*(1.-fr_snow(ibv)*fm)*evapdl(n,ibv) )*dts

         if (wn < dz(n)*thetm(n,ibv)) then
            f(n+1,ibv) = max(f(n+1,ibv), 0.d0) !no soil-to-aquifer flux
         endif  
      enddo
...
ccc   Prevent over/undersaturation of layers 2:n
     ...
     &       - rnff(k,ibv) - fd*(1.-fr_snow(ibv)*fm)*evapdl(k,ibv) )*dts

ccc   Compensate oversaturation by increasing flux up
          if (wn - ws(k,ibv) > 0.d0) then
              f(k,ibv) = f(k,ibv) + (wn - ws(k,ibv)) / dts
          endif

          wn = w(k,ibv) + ( f(k+1,ibv) - f(k,ibv)
     &       - rnff(k,ibv) - fd*(1.-fr_snow(ibv)*fm)*evapdl(k,ibv) )*dts

ccc   Compensate oversaturation by increasing underground runoff
          if ( wn - ws(k,ibv) > 0.d0) then
            rnff(k,ibv) = rnff(k,ibv) + (wn - ws(k,ibv))/dts
          endif
          
ccc   Compensate undersaturation by eliminating runoff          
          if (wn - dz(k)*thetm(k,ibv) < 0.d0) then
             rnff(k, ibv) = 0.d0
ccc   Check if this was enough
             wn = w(k,ibv) + ( f(k+1,ibv) - f(k,ibv)
     &       - rnff(k,ibv) - fd*(1.-fr_snow(ibv)*fm)*evapdl(k,ibv))*dts
ccc   If not, compensate undersaturation by eliminating upward flux
             if (wn - dz(k)*thetm(k,ibv) < 0.d0) then
               f(k,ibv) = min(f(k,ibv), 0.d0)
             endif
             wn = w(k,ibv) + ( f(k+1,ibv) - f(k,ibv)
     &       - rnff(k,ibv) - fd*(1.-fr_snow(ibv)*fm)*evapdl(k,ibv))*dts
ccc   Are we OK now? If not,emit a warning;set fluxes below this layer to 0    
             if (wn - dz(k)*thetm(k,ibv) < 0.d0) then
!               write(99,*) "GHY:FLLMT:warning:set water fluxes to 0"
!     &                       , ijdebug
                f((k+1):(n+1), ibv) = 0.d0
                rnff((k+1):n, ibv) = 0.d0
             endif
...
ccc   Same thing for the top layer
      do ibv=i_bare,i_vege

         wn = w(1,ibv) + ( f(2,ibv) - f(1,ibv) - rnf(ibv)
     &      - rnff(1,ibv) - fd*(1.-fr_snow(ibv)*fm)*evapdl(1,ibv))*dts

ccc   Compensate undersaturation by eliminating runoff          
         if (wn - dz(1)*thetm(1,ibv) < 0.d0) then
           rnf(ibv) = 0.d0
           rnff(1,ibv) = 0.d0
ccc   Check if this was enough
           wn = w(1,ibv) + ( f(2,ibv) - f(1,ibv) - rnf(ibv)
     &         - rnff(1,ibv) 
     &         - fd*(1.-fr_snow(ibv)*fm)*evapdl(1,ibv) )*dts
ccc   Are we OK now? If not, emit a warning and set all column fluxes to 0
           if (wn - dz(1)*thetm(1,ibv) < 0.d0) then
!              write(99,*) "GHY:FLLMT:warn:set layer1 waterfluxes to 0"
!     &                       , ijdebug
              f(2:n, ibv) = 0.d0
              rnff(2:n, ibv) = 0.d0
c            Recalculate the water content    
              wn = w(1,ibv) + ( f(2,ibv) - f(1,ibv) - rnf(ibv)
     &            - rnff(1,ibv) 
     &            - fd*(1.-fr_snow(ibv)*fm)*evapdl(1,ibv) )*dts
           endif
...
ccc   Compensate oversaturation by increasing surface runoff
         if ( wn - ws(1,ibv) > 0.d0) then
!            write(99,*) "FLLMT:warning:oversaturated layer 1"
!     &                       , ijdebug, ibv
            rnf(ibv) = rnf(ibv) + (wn - ws(1,ibv))/dts
         endif

      enddo

ccc   UNNECESSARY (?) Compensate oversaturation by increasing runoff
      do ibv=i_bare,i_vege
        do k=2,n
          wn = w(k,ibv) + ( f(k+1,ibv) - f(k,ibv)
     &       - rnff(k,ibv) - fd*(1.-fr_snow(ibv)*fm)*evapdl(k,ibv) )*dts
...
c#ifdef AQUIFER
c**** dz - soil layer thicknesses, m
c**** h_aq - aquifer water table (=head), m
c**** fice_aq - fraction of ice in aquifer
c**** xku - conductivity of layers, m s-1 
c**** (xk is conductivity estimated at the interface between layers)
c#endif
c**** output:
...
c****
      ...
      do ibv=i_bare,i_vege
#ifdef AQUIFER
#ifdef AQUIFER_NO_WATER_FLUX
        f(n+1,ibv)=0.d0
#else
c**** flow from the aquifer to the bottom soil layer
        f(n+1,ibv)=2.d0*xku(n,ibv)*(h_aq-h(n,ibv))/dz(n)
#endif
#else
c**** no-flow bottom boundary condition
        f(n+1,ibv)=0.d0
#endif
      end do
c****
...
c#ifdef AQUIFER
c**** dz - soil layer thicknesses, m
c**** tp_aq - aquifer temperature, degC
c#endif
c**** output:
...
c****
      ...
      do ibv=i_bare,i_vege
#ifdef AQUIFER
c**** heat flow from the aquifer to the bottom soil layer
c**** conduction term
#ifdef AQUIFER_NO_HEAT_FLUX
        fh(n+1,ibv)=0.d0
#else	
        fh(n+1,ibv)=2.d0*xkh(n,ibv)*(tp_aq-tp(n,ibv))/dz(n)
#endif
c**** limit amount of heat conducted for numerical stability
        heatcap = min(shc_aq+w_aq*shw, shc(n,ibv)+w(n,ibv)*shw)
        if (abs(fh(n+1,ibv))*dts .gt.  
     &      abs(tp_aq-tp(n,ibv))*heatcap) then
                fh(n+1,ibv) = (tp_aq-tp(n,ibv))*heatcap/dts            
        endif
        
c**** advection term
        if (f(n+1,ibv) .gt. 0.d0) then !flow from aquifer into soil
          fh(n+1,ibv)=fh(n+1,ibv)+f(n+1,ibv)*tp_aq*shw
        else
          fh(n+1,ibv)=fh(n+1,ibv)+f(n+1,ibv)*tp(n,ibv)*shw
        endif          

#else
c**** no-flux bottom boundary condition
        fh(n+1,ibv)=0.d0
#endif
...
c#ifdef AQUIFER
c**** w_aq - aquifer water volume per unit surface area (m)
c**** ht_aq - aquifer water heat content per unit surface area (J/m^2)
c#endif
c**** output:
c**** tp - temperature of layers, c
c**** fice - fraction of ice of layers
c#ifdef AQUIFER
c**** shc_aq - aquifer rock specific heat capacity (J/m^2 per K) 
c**** tp_aq - aquifer temperature, degC
c**** fice_aq - aquifer fraction frozen
c#endif
...
#ifdef AQUIFER
c**** initialize temperature and ice fraction
      tp_aq = 0.d0
      fice_aq = 0.d0
c**** now modify these as appropriate
      if ( fsn*w_aq+ht_aq .lt. 0.d0 ) then ! all frozen
        tp_aq=(ht_aq+w_aq*fsn)/(shc_aq+w_aq*shi)
        fice_aq=1.d0
      else if ( ht_aq .gt. 0.d0 ) then ! all melted
        tp_aq=ht_aq/(shc_aq+w_aq*shw)
      else if ( w_aq .ge. 1d-12 ) then  ! part frozen
        fice_aq=-ht_aq/(fsn*w_aq)
      endif      
#endif
...
!@sum apply computed fluxes to advance w and h
!@ver 1.0
c**** shw - specific heat of water
c**** tp - temperature of layers, c
      integer ibv, k
...
#ifdef AQUIFER
ccc   advance the aquifer water and heat contents based on the computed fluxes
      w_aq = w_aq - dts*ptype*(fb*f(n+1,i_bare) + fv*f(n+1,i_vege))
      ht_aq = ht_aq - dts*ptype*(fb*fh(n+1,i_bare) + fv*fh(n+1,i_vege))        
c**** we multiply by ptype to adjust for not including flow
c**** to/from the aquifer 
c**** from ice and lake fractions of the grid cell
#endif

ccc check for under/over-saturation
      do ibv=i_bare,i_vege
        do k=1,n
          if ( dz(k)*thetm(k,ibv) - w(k,ibv) >  1.d-14 ) then
c            Print out variables to find error
             write(933,*) 'Check in apply_fluxes'
             write(933,*) 'ijdebug=',ijdebug
             write(933,*)'ibv, k=',ibv,k
             write(933,*)'new w(k,ibv)=',w(k,ibv)
             write(933,*)'wmin=',dz(k)*thetm(k,ibv)
             write(933,*)'diff=',w(k,ibv) - dz(k)*thetm(k,ibv)
             write(933,*)'f(k+1,ibv)*dt=',f(k+1,ibv)*dts
             write(933,*)'f(k,ibv)*dt=',f(k,ibv)*dts
             write(933,*)'rnff(k,ibv)*dts',rnff(k,ibv)*dts
             write(933,*)'ET*dts',fd*(1.-fr_snow(ibv)*fm)*evapdl(k,ibv)
     &        *dts
             write(933,*)'evapvg*dts',evapvg*(1.d0-fr_snow(2))*dts
             write(933,*)'snowmelt',
     &        (flmlt(ibv)*fr_snow(ibv) + flmlt_scale(ibv))*dts
             write(933,*)'dripw', dripw(ibv)*(1.d0-fr_snow(ibv))*dts
             write(933,*)'irrigation', irrig(ibv)*dts            
             print*, 'STOPPING:diff',dz(k)*thetm(k,ibv)- w(k,ibv)
c            Stop model
             call stop_model("ghy: w < dz*thetm",255)

          else
c            Ignore machine error and make sure w > minimum
             w(k,ibv) = max( w(k,ibv), dz(k)*thetm(k,ibv) )
          end if
          
          if ( w(k,ibv) - ws(k,ibv) > 1.d-14 ) then
c            Print out variables to find error
             write(933,*) 'Check in apply_fluxes'
             write(933,*) 'ijdebug=',ijdebug
             write(933,*)'ibv, k=',ibv,k
             write(933,*)'new w(k,ibv)=',w(k,ibv)
             write(933,*)'ws=',ws(k,ibv)
             write(933,*)'wmin=',dz(k)*thetm(k,ibv)
             write(933,*)'diff=',w(k,ibv)-ws(k,ibv)
             write(933,*)'f(k+1,ibv)*dt=',f(k+1,ibv)*dts
             write(933,*)'f(k,ibv)*dt=',f(k,ibv)*dts
             write(933,*)'rnf(ibv)*dts',rnf(ibv)*dts
             write(933,*)'rnff(k,ibv)*dts',rnff(k,ibv)*dts
             write(933,*)'ET*dts',fd*(1.-fr_snow(ibv)*fm)*evapdl(k,ibv)
     &        *dts
             write(933,*)'evapvg*dts',evapvg*(1.d0-fr_snow(2))*dts
     ...
     &        (flmlt(ibv)*fr_snow(ibv) + flmlt_scale(ibv))*dts
             write(933,*)'dripw', dripw(ibv)*(1.d0-fr_snow(ibv))*dts
             write(933,*)'irrigation', irrig(ibv)*dts            
             write(933,*) 'STOPPING:diff',w(k,ibv)-ws(k,ibv)
             print*,'w(k,ibv)',w(k,ibv),k,ibv
             print*,'ws(k,ibv)',ws(k,ibv)
c            Stop model
             call stop_model("ghy: w > ws",255)
          else
c            Ignore machine error and make sure w < maximum
...
#ifdef AQUIFER
c**** determine bottom level of aquifer
      zb_aq = zb(n+1) - dz_aq
c**** determine level of aquifer water table
c**** assuming constant aquifer specific yield
      h_aq = zb_aq + w_aq/Sy_aq
#endif
...
#ifdef AQUIFER
c**** heat capacity of aquifer regolith/rock
c**** assumed to be same as bedrock (2.4E6 J/m^3 /K);
c**** heat capacity of air-filled pore space is neglected
c**** NOTE: IF CHANGED BELOW, MAKE CORRESPONDING CHAGES IN GHY_DRV.f
      shc_aq = (1.d0 - Sy_aq) * dz_aq * 2.4d6
#endif
...
!-----------------------------------------------------------------------
     ...
     &     top_index_in,top_stdev_in,dz_in,q_in,qk_in,sl_in, ! constants
     &     fb_in,fv_in,               ! constants
...
c#ifdef AQUIFER
c**** w_aq - aquifer water, m
c**** ht_aq - heat in aquifer, J/m^2
c**** ptype - fearth for grid cell (1 minus land ice fraction minus lake fraction)
c**** (used for computing fraction of aquifer surface area exchanging with soil)
c#endif
c**** output:
c**** w - updated water in soil layers, m s-1
c**** ht - updated heat in soil layers
c#ifdef AQUIFER
c**** w_aq - updated aquifer water, m
c**** ht_aq - updated heat in aquifer, J/m^2
c#endif
...
#ifdef AQUIFER
      real*8 :: w_aq_in,ht_aq_in,ptype_in       
#endif
      integer :: nsn_in(2)
...
!          write(933,*) "ent_forcings",ts-tfrz,tp(0,2),Qf,pres,Ca,ch,vs,
!     &         vis_rad,direct_vis_rad,cosz1,sbgc_temp,sbgc_moist,
!     &         h(1:ngm,2),fice(1:ngm,2) 
     ...
     &         surf_pressure=pres,
!     &         surf_CO2=Ca, ! fol_CO2=1.d30, (For lsm_standalone, 2010)
     &         surf_CO2=Ca*(1.0D-06)*pres*100.0/gasc/(tp(0,2)+tfrz),!(for current ModelE)
 !    &       precip=pr,
...
ccc unpack necessary data
     ...
     &         )
        ...
        call evap_limits(dts, .true., dum1, dum2 )
#else
        call evap_limits(dts, vegcell, .true., dum1, dum2 )
#endif
...
!     Fluxes between soil layers m/s for each fraction
      accum_water_flux(:,:) = f(:,:)!accum_water_flux(:,:) + f(:,:) *dts
      accum_heat_flux(:,:) =  fh(:,:)!accum_heat_flux(:,:) + fh(:,:)*dts
...
c accumulations are collected.
      ...
      aepc=rhow*aepc
      aepb=rhow*aepb
      accum_water_flux(:,:) = rhow*accum_water_flux(:,:)
      accum_heat_flux(:,:) =  rhow*accum_heat_flux(:,:)
...
c zero out accumulations
      ...
      aflmlt=0.d0
      aintercep=0.d0
      accum_water_flux(:,:) = 0.d0
      accum_heat_flux(:,:) =  0.d0
...
c**** soils28   common block     9/25/90
      ...
      write(ichn,*)'dts,tag',dts,i,' after retp(1),advnc(2)'
cc    write(ichn,1021)
      write(ichn,1045)
...
#ifdef AQUIFER
ccc if the aquifer water table is much below the soil bottom,
ccc then return the aquifer water table
      if (h_aq < (zb(n+1) - 1.d-2)) then
        zw(:) = h_aq
      endif
#endif 
...
#ifdef AQUIFER
ccc add aquifer water to total to conserve water
        total_water(:) = total_water(:) + w_aq/ptype
#endif 
...
#ifdef AQUIFER
ccc note: unlike before,
ccc water within fractions of grid cell is *not* conserved,
ccc under current aquifer implementation,
ccc only the total water in the grid cell

      error_water = fb * ((total_water(1) 
     &  - old_total_water(1))/dts
     &  - irrig(1) + evap_tot(1) 
     &  + sum(rnff(1:n, 1)) + rnf(1))
     &  + fv * ((total_water(2) 
     &  - old_total_water(2))/dts
     &  - irrig(2) + evap_tot(2) 
     &  + sum(rnff(1:n, 2)) + rnf(2))
     &  - pr

      if (abs(error_water)>1.d-15)
     &     write(99,*)'combined(AQUIFER) error_water:',ijdebug
     &     ,error_water
      if ( abs( error_water ) > 1.d-10 ) then
        write(0,*)'sum(evap_tot(1:2))',ijdebug,sum(evap_tot(1:2))
c       call stop_model(
c    &     'GHY: water conservation problem in soil',255)
      endif
#else
      ! bare soil
      if ( process_bare ) then
...
#ifdef AQUIFER
ccc add aquifer heat to total to conserve energy
      total_energy(:) = total_energy(:) + ht_aq/ptype
#endif 
...
#ifdef AQUIFER
ccc note: unlike before,
ccc energy within fractions of grid cell is *not* conserved,
ccc under current aquifer implementation,
ccc only the total water in the grid cell
      error_energy = fb * ( (total_energy(i_bare) 
     &  - old_total_energy(i_bare))/dts
     &  - htirrig(1) + elh*evap_tot(1)
     &  + shw*(sum(rnff(1:n,1)*max(tp(1:n,1),0.d0) ) )
     &  + shw*rnf(1)*max(tp(1,1),0.d0) 
     &  + thrm_tot(1) + snsh_tot(1) )
     &  + fv * ((total_energy(2) 
     &  - old_total_energy(2))/dts
     &  - htirrig(2) + elh*evap_tot(2) 
     &  + shw*( sum(rnff(1:n,2)*max(tp(1:n,2),0.d0) ) )
     &  + shw*rnf(2)*max(tp(1,2),0.d0) 
     &  + thrm_tot(2) + snsh_tot(2))
     &  - htpr - srht - trht

       if ( abs( error_energy ) > 1.d-5 ) then
          write(99,*)'GHY:combined(AQUIFER) error_energy',ijdebug,
     &                error_energy
...
c**** and by avoiding unnecessary calls to hydra.  also eliminates call
