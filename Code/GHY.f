...
!@sum  Details of model physics are in the following publications: 
!@sum  1)Abramopoulos etal (1988).J of Climate,1(9),921-941.
!@sum  2)Rosenzweig & Abramopoulos.(1997).J of Climate,10(8),2040-2054. 
!@sum  3)Aleinov & Schmidt (2006).Global&Planetary Change,51(1),108-120.
!@sum  4)Puma etal (2013).JGR: Biogeosciences,118(1),14-29.
!@sum  5)Schmidt (2014).J of Advances in Modeling Earth Systems,6(1),141-184.

!@auth I. Aleinov, M.J. Puma, et al. (2016 to present)
!-----------------------------------------------------------------------
...
!     fr(k) is the fraction of roots in layer k
        ...
        write(933,*) 'in evap_limits:'
        write(933,*) 'betad= ',betad
        write(933,*) 'evapvd= ',evapvd
        write(933,*) 'betadl(k)= ',betadl(1:n)
        write(933,*) 'betadl(k)/betad ',betadl(1:n)/betad
...
c     bare soil evaporation
      if ( process_bare ) then
!      Keep surface evaporation to the available water in the top soil layer       
        evap_max_dry(i_bare) = min(evap_max_dry(i_bare),
     &    (w(1,i_bare)-dz(1)*thetm(1,i_bare))/dt)
...
!      Keep surface evaporation to the available water in the top soil layer 
     ...
     &   (w(1,i_vege)-dz(1)*thetm(1,i_vege))/dt-evapdl(1,i_vege))
...
!     &     ( theta(0,2)*evapvw + (1.-theta(0,2))*evapvd )
      ...
      ! Bare soil: set vegetation-related ET fluxes to zero
...
!     Compute transpiration for separate layers (=0 for bare soil)
      ...
      if ( betad > 0.d0 ) then
         do k=1,n
            ...
            write(933,*) 'in evap_limits: layer # ',k
            write(933,*) 'betad= ',betad
            write(933,*) 'evapvd= ',evapvd
            write(933,*) 'betadl(k)= ',betadl(k)
            write(933,*) 'betadl(k)/betad ',betadl(k)/betad
         end do
      endif

!     Reduce water taken from soil layers so that soil water does not 
!     drop below its minimum value (Nir, 6/2011)
      do ibv=i_bare,i_vege
         do k=1,n
     ...
     &                        (w(k,ibv)-dz(k)*thetm(k,ibv))/dt)
         enddo
      enddo
!     Adjust the total evaporation accordingly  
      evapvd = 0.d0
      do k=1,n
...
c**** surface runoff
      do ibv=i_bare,i_vege
        ! Niu et al 2006: max satfrac ~0.5
        satfrac = (w(1,ibv)/ws(1,ibv))**rosmp
        satfrac = min( satfrac, 0.5d0)
...
c**** Revised: Nir Krakauer / M.J. Puma, 2017
c**** Input:
...
c            Print out variables to find error
             ...
             write(933,*)'betadl(k,ibv)',betadl(k)
             write(933,*)'betad',betad
             write(933,*)'evapdl(k,ibv)',evapdl(k,ibv)
...
ccc unpack necessary data
     ...
     &         )
       ...
       write(933,*) "before evap_limits in subroutine advnc:"
       write(933,*) "w             ", w             
       write(933,*) "ht            ", ht            
       write(933,*) "nsn           ", nsn           
...
#ifdef PRINT_GHY_VARS
       write(933,*) "towards end of subroutine advnc:"
       write(933,*) "w             ", w             
       write(933,*) "ht            ", ht            
       write(933,*) "nsn           ", nsn           
       !write(933,*) "dzsn(1:nlsn,:)", dzsn(1:nlsn,:)
